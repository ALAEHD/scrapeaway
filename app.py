...
# Define Celery tasks for each scraper
@celery.task
def run_scraper(scraper_name, query):
    scraper_func = SCRAPER_MAP.get(scraper_name)
    if scraper_func:
...
def index():
    if request.method == "POST":
        ...
        # Create a Celery group of tasks to run all selected scrapers concurrently
        task_group = group(run_scraper.s(scraper_name, query) for scraper_name in selected_scrapers)
        result = task_group.apply_async()

        # Store the task ID in the session to retrieve the results later
...
def results():
    ...
    if task_id:
        ...
        if result.ready():
            ...
            flash('Scraping in progress. Please check back later.', 'info')
